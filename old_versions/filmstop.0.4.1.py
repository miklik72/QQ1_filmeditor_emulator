...
# v0.4.1, 26.03.2019, Martin Mikala, read stop point from file name
...
for fpath in files:
	...
	if debug:
		print(fname)
		print(ftime)
	VIDEO_ACTION.append([int(ftime), fpath])
	...
	if time_current - time_last > time_jump:
		...
		if pushP != lastP:             #chanche Play/Pause button
			...
			if  not (activeP or activeF or activeR):					# any button pushed
				...
				for A in VIDEO_ACTION:         # check all action points
					DELTA = abs(VIDEO_POSITION - float(A[0]/100))
					if debug:
					    print("position %f, action %f" % (player.position(), A[0]/100))
					if DELTA <= ACTION_TOLERANCE:
						if debug:
							#print("Catch action point, position %f, I %s file %s" % (player.position(),str(I),A[I][1]))
